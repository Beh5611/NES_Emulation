...
class P6502
{
    ...
    // private:
        uint8_t acc, x, y, flag_status; // Accumilator and the Registers
        ...
        {
            std::string name;		
            uint8_t     (P6502::*operation )(void) = nullptr;
            uint8_t     (P6502::*address_mode)(void) = nullptr;
            uint8_t     cycles = 0;
        };

        // Addressing Modes
        uint8_t IMP(), IMM(), ZP0(), ZPX(), ZPY(), REL();
	    uint8_t ABS(), ABX(), ABY(), IND(), IZX(), IZY();

        

        // OPcodes
        uint8_t ADC(), AND(), ASL(), BCC();
        uint8_t BCS(), BEQ(), BIT(), BMI();
        uint8_t BNE(), BPL(), BRK(), BVC();
        uint8_t BVS(), CLC(), CLD(), CLI();
        uint8_t CLV(), CMP(), CPX(), CPY();
        uint8_t DEC(), DEX(), DEY(), EOR();
        uint8_t INC(), INX(), INY(), JMP();
        uint8_t JSR(), LDA(), LDX(), LDY();
        uint8_t LSR(), NOP(), ORA(), PHA();
        uint8_t PHP(), PLA(), PLP(), ROL();
        uint8_t ROR(), RTI(), RTS(), SBC();
        uint8_t SEC(), SED(), SEI(), STA();
        uint8_t STX(), STY(), TAX(), TAY();
        uint8_t TSX(), TXA(), TXS(), TYA();

        // Catcher for unofficial opcodes, same as NOP()
        uint8_t XXX();
